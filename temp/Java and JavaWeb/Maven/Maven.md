...
2. 对于服务器，没有 IDE 时（脱离本地开发环境），就需要 Maven 来将程序构建、打包和部署：
    ![images](Maven.assets/img010.74e515e5.png)

### 3. Maven 工作机制

1. 总体图：
    ![./images](Maven.assets/img003.f9cc536c.png)
...
6. 用命令生成 maven 工程
    ...
    ![images](Maven.assets/img008.be45c9ad.png)
...
4. 约定的目录结构（在超级 pom 中配置）![./images](Maven.assets/img011.621b1ac3.png)
...
1. 使用 `mvn archetype:generate` 命令生成 Web 工程时，需要使用一个专门的 archetype。这个专门生成 Web 工程骨架的 archetype 可以参照官网看到它的用法：
    ![./images](Maven.assets/img014.942770a3.png)
...
2. 当为了避免 jar 包之间的冲突时，需要用到依赖的排除，例如：
    ![./images](Maven.assets/img027.2faff879.png)
...
1. IDEA 右边的工具栏中，找到 Maven 点开，到具体的 module，然后里面有 Lifecycle 文件夹，点开，双击里面的命令就会执行。
    ![image-20230130134515513](Maven.assets/image-20230130134515513.png)

2. 同样的，Maven 内还有 `m` 的图表，点击后可以输入执行命令，同时也可以指定命令执行的范围。

    ![](Maven.assets/download.png)
...
3. 生成 web.xml：在 Facets 的具体模块中，在 Deployment Descriptors（部署描述符） 中添加，注意目录要改：
    ![./images](Maven.assets/img046.71c20d43.png)
...
2. 目标：
    ...
    ![image-20230130163558304](Maven.assets/image-20230130163558304.png)
