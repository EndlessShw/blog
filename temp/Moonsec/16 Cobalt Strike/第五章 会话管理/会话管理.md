...
1. 首先在 metasploit 中创建监听（这里使用 reverse_tcp ）：
    ...
    ![image-20220124112805508](会话管理.assets/image-20220124112805508.png)

     补充：

    jobs 命令可以查看后台的任务项。例如：

    ![image-20220124112902547](会话管理.assets/image-20220124112902547.png)

    使用 `kill <id>` 可以关闭某个任务

2. 在 cobalt strike 中新建监听器，这里根据上文的监听类型，选择对应的监听器种类：

    ![image-20220124113114337](会话管理.assets/image-20220124113114337.png)
...
2. 在 cobalt strike 中设置监听器。这里的监听器为 windows/beacon_http/reverse_http
    ...
    ![image-20220124153005256](会话管理.assets/image-20220124153005256.png)
...
4. 结果如下：
    ...
    ![image-20220125205227167](会话管理.assets/image-20220125205227167.png)
